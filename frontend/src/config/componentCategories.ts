...
export function getAllCategoryIds(): string[] {
  return COMPONENT_CATEGORIES.map(cat => cat.id);
}

// Helper function to get subcategories for a category
export function getSubcategories(categoryId: string): SubcategoryDefinition[] {
  const category = COMPONENT_CATEGORIES.find(cat => cat.id === categoryId);
  return category?.subcategories || [];
}

// Helper function to get applications for a subcategory
export function getApplications(categoryId: string, subcategoryId: string): string[] {
  const category = COMPONENT_CATEGORIES.find(cat => cat.id === categoryId);
  const subcategory = category?.subcategories.find(sub => sub.id === subcategoryId);
  return subcategory?.applications || [];
}
...
// Helper function to flatten all categories for search
export function flattenCategories(): { category: string; subcategory: string; application?: string }[] {
  const result: { category: string; subcategory: string; application?: string }[] = [];
